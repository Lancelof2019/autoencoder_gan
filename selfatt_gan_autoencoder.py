...
class encoderNetwork(tf.keras.models.Model):
    ...
    def call(self, inputs, is_train):
        ...
        model = BatchNormalization()(l3, training=bool(self.is_train))
...
class Autoencoder(tf.keras.models.Model):
    ...
    def lossfun(self, sampleInput, is_train, init, logger):
        ...
        print("sampleInput type:", type(sampleInput))
        ...
        self.encoded, z_mean, z_log_var, z = self.encoder(attention_output, self.is_train)
        ...
        reconstruction_loss = 0.5 * error + 0.5 * self.lamda * (
                    1 - self.alpha) * sgroup_lasso + 0.5 * self.lamda * self.alpha * lasso
    ...
    def testprocess(self, inputs, iterator, params):
        ...
        while epoch < self.max_epochs and stop == False:
            ...
            with tf.GradientTape(persistent=True) as tape_ae, tf.GradientTape() as tape_d:
                total_loss, reconstruction_loss, discriminator_loss, generator_loss = self.lossfun(inputs,
                                                                                                   self.is_train,
                                                                                                   self.init,
                                                                                                   logger=None)
        ...
        # costs=costs.numpy()
        with open(
                r'../data/python_related/result/community/communityScores_compare_' + str(cancer_type) + "_" + str(
                    iterator) + '_test14.csv',
        ...
        plt.title("Learning rate =" + str(round(self.learning_rate, 9)) + "@" + str(cancer_type) + " " + str(
            self.n_input1) + "," + str(self.n_input2) + "@" + str(self.n_hidden1) + "," + str(self.n_hidden2) + "\n")
        plt.savefig(r'../figure/loss_curve/getscores_picture_' + str(cancer_type) + "_" + str(iterator) + '_test14.png')
        plt.close()
